...
def update_window(filename, text_widget):
    with open(f"scandata/{filename}", "r") as f:
        contents = f.read()
...
def save_to_file(filename, text_widget):
    ...
    with open(f"scandata/{filename}", "w") as f:
        f.write(contents)

def check_for_changes(filename, text_widget):
    # get the current modification time of the file
    current_mtime = os.stat(f"scandata/{filename}").st_mtime
...
# get a list of all the filenames in the scandata directory
filenames = os.listdir("scandata")
...
for i, filename in enumerate(filenames):
    ...
    text.last_mtime = os.stat(f"scandata/{filename}").st_mtime
